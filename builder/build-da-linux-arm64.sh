#!/usr/bin/env bash
# =========================================================================
#  build-da-linux-arm64.sh
#  Builds  da  +  ollama-main  for Linux aarch64 (ARM64).
#  Output → bin/dev-assist/
#
#  Full isolation: fresh venv, no --system-site-packages.
#  Usage: bash builder/build-da-linux-arm64.sh
# =========================================================================
set -euo pipefail
...
VENV_DIR="$SCRIPT_DIR/.venv-da-arm64"
OUT_DIR="$PROJECT_ROOT/bin/dev-assist"

echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo "  dev-assist — Linux ARM64 Build  (da + ollama-main)"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
mv "$DA_DIR/dist/da-arm64" "$OUT_DIR/da-arm64"
chmod +x "$OUT_DIR/da-arm64"
ok "da-arm64 → $OUT_DIR/da-arm64  ($(du -sh "$OUT_DIR/da-arm64" | cut -f1))"

# ── Build ollama-main ─────────────────────────────────────────────────────
info "Building ollama-main-arm64..."
rm -rf "$DA_DIR/build" "$DA_DIR/dist" "$DA_DIR/__pycache__" "$DA_DIR"/*.spec

"$PYINSTALLER_BIN" \
    --onefile \
    --name ollama-main-arm64 \
    --clean \
    "${OLLAMA_HIDDEN[@]}" \
    "${OLLAMA_EXCLUDED[@]}" \
    ollama-main/main.py

[[ -f "$DA_DIR/dist/ollama-main-arm64" ]] || die "Build failed — ollama-main-arm64 not found"
mv "$DA_DIR/dist/ollama-main-arm64" "$OUT_DIR/ollama-main-arm64"
chmod +x "$OUT_DIR/ollama-main-arm64"
ok "ollama-main-arm64 → $OUT_DIR/ollama-main-arm64  ($(du -sh "$OUT_DIR/ollama-main-arm64" | cut -f1))"
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo "  ✅ dev-assist ARM64 build complete"
echo ""
echo "  bin/dev-assist/"
echo "    ├── da-arm64            $(du -sh "$OUT_DIR/da-arm64" | cut -f1)"
echo "    └── ollama-main-arm64   $(du -sh "$OUT_DIR/ollama-main-arm64" | cut -f1)"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
